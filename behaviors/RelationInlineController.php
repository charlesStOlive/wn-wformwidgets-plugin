<?php namespace  Waka\Wformwidgets\Behaviors;
 ...
 *
 *     public $implement = [
 *         \Backend\Behaviors\RelationInlineController::class,
 *     ];
 *
 *     public $relationInlineConfig = 'config_inline_relation.yaml';
 *
 * The `$relationInlineConfig` property makes reference to the configuration
 ...
 */
class RelationInlineController extends ControllerBehavior
{
     ...
     */
    const PARAM_FIELD = '_relation_inline_field';

    /**
     * @var const Postback parameter for the active management mode.
     */
    const PARAM_MODE = '_relation_inline_mode';

    /**
     * @var const Postback parameter for read only mode.
     */
    const PARAM_EXTRA_CONFIG = '_relation_inline_extra_config';
    ...
    public function __construct($controller)
    {
        ...
        $this->addJs('js/winter.relation_inline.js', 'waka.wformwidgets');
        $this->addJs('js/winter.popupInline.js', 'waka.wformwidgets');
        $this->addCss('css/relation_inline.css', 'waka.wformwidgets');

        /*
         * Build configuration
         */
        $this->config = $this->originalConfig = $this->makeConfig($controller->relationInlineConfig ?: $this->relationInlineConfig, $this->requiredConfig);
    ...
    protected function validateField($field = null)
    {
        ...
        if ($field && $field != $this->field) {
            $this->initInlineRelation($this->model, $field);
        }
    ...
    public function prepareVars()
    {
        ...
        $this->vars['relationSessionKey'] = $this->relationInlineGetSessionKey();
     ...
     * supply the relation's field name (_relation_inline_field).
     */
     ...
     * @return void
     */
    public function initInlineRelation($model, $field = null)
    {
        //trace_log('initInlineRelation-- with modelName '.$model?->name ?? 'pas acces');
        if ($field == null) {
        ...
        if ($this->manageFilterWidget = $this->makeFilterWidget('manage')) {
            $this->controller->relationInlineExtendManageFilterWidget($this->manageFilterWidget, $this->field, $this->model);
            $this->manageFilterWidget->bindToController();
        }

        if ($this->viewFilterWidget = $this->makeFilterWidget('view')) {
            $this->controller->relationInlineExtendViewFilterWidget($this->viewFilterWidget, $this->field, $this->model);
        ...
        if ($this->viewWidget = $this->makeViewWidget()) {
            $this->controller->relationInlineExtendViewWidget($this->viewWidget, $this->field, $this->model);
        ...
        if ($this->manageWidget = $this->makeManageWidget()) {
            $this->controller->relationInlineExtendManageWidget($this->manageWidget, $this->field, $this->model);
        ...
        if ($this->manageMode === 'pivot' && $this->pivotWidget = $this->makePivotWidget()) {
            $this->controller->relationInlineExtendPivotWidget($this->pivotWidget, $this->field, $this->model);
     ...
     */
    public function relationInlineRender($field, $options = [])
    {
        //trace_log('relationInlineRender-- model Name '.$this->model?->name ?: 'error ');
        /*
         * Session key
         */
        ...
        switch (strtolower($section)) {
            ...
            case 'view':
                return $this->relationInlineMakePartial('view');

            default:
                return $this->relationInlineMakePartial('container');
        }
    }
     ...
     */
    public function relationInlineRefresh($field = null)
    {
        $field = $this->validateField($field);

        $result = ['#'.$this->relationInlineGetId('view') => $this->relationInlineRenderView($field)];
        if ($toolbar = $this->relationInlineRenderToolbar($field)) {
            $result['#'.$this->relationInlineGetId('toolbar')] = $toolbar;
        }

        if ($eventResult = $this->controller->relationInlineExtendRefreshResults($field)) {
     ...
     */
    public function relationInlineRenderToolbar($field = null)
    {
        return $this->relationInlineRender($field, ['section' => 'toolbar']);
    }
     ...
     */
    public function relationInlineRenderView($field = null)
    {
        return $this->relationInlineRender($field, ['section' => 'view']);
    }
     ...
     */
    public function relationInlineMakePartial($partial, $params = [])
    {
        $contents = $this->controller->makePartial('relation_inline'.$partial, $params + $this->vars, false);
        if (!$contents) {
     ...
     * @return string
     */
    public function relationInlineGetId($suffix = null)
    {
     ...
     */
    public function relationInlineGetSessionKey($force = false)
    {
    ...
    protected function makeToolbarWidget()
    {
        ...
        if (!$this->readOnly && $this->toolbarButtons) {
            $defaultButtons = '~/plugins/waka/wformwidgets/behaviors/relationinlinecontroller/partials/_toolbar.php';
        }
    ...
    protected function makeViewWidget()
    {
        ...
        if ($this->viewMode === 'multi') {
            ...
            $config->showTree = $this->getConfig('view[showTree]');
            $config->noRecordsMessage = $this->getConfig('view[noRecordsMessage]');

            $defaultOnClick = sprintf(
                "$.wn.relationInlineBehavior.clickViewListRecord(':%s', '%s', '%s')",
                $this->relationModel->getKeyName(),
                $this->relationInlineGetId(),
                $this->relationInlineGetSessionKey()
            );
            ...
            $widget->bindEvent('list.extendQuery', function ($query) {
                ...
                $sessionKey = $this->deferredBinding ? $this->relationInlineGetSessionKey() : null;
        ...
        if ($this->manageMode === 'list' || $this->manageMode === 'pivot') {
            ...
            if ($this->viewMode === 'single') {
                ...
                $config->recordOnClick = sprintf(
                    "$.wn.relationInlineBehavior.clickManageListRecord(':%s', '%s', '%s')",
                    $this->relationModel->getKeyName(),
                    $this->relationInlineGetId(),
                    $this->relationInlineGetSessionKey()
                );
            ...
            elseif ($config->showCheckboxes) {
                $config->recordOnClick = "$.wn.relationInlineBehavior.toggleListCheckbox(this)";
            }
            elseif ($isPivot) {
                $config->recordOnClick = sprintf(
                    "$.wn.relationInlineBehavior.clickManagePivotListRecord(':%s', '%s', '%s')",
                    $this->relationModel->getKeyName(),
                    $this->relationInlineGetId(),
                    $this->relationInlineGetSessionKey()
                );
    ...
    public function onRelationInlineInlineButtonAdd()
    {
        $this->eventTarget = 'button-add';

        return $this->onRelationInlineInlineManageForm();
    }

    public function onRelationInlineButtonCreate()
    {
        $this->eventTarget = 'button-create';

        return $this->onRelationInlineManageForm();
    }

    public function onRelationInlineButtonDelete()
    {
        return $this->onRelationInlineManageDelete();
    }

    public function onRelationInlineButtonLink()
    {
    ...
    public function onRelationInlineButtonRefresh()
    {
        $this->beforeAjax();
        return $this->relationInlineRefresh();
    }

    public function onRelationInlineButtonUnlink()
    {
        return $this->onRelationInlineManageRemove();
    }

    public function onRelationInlineButtonRemove()
    {
        return $this->onRelationInlineManageRemove();
    }

    public function onRelationInlineButtonUpdate()
    {
    ...
    public function onRelationInlineClickManageList()
    {
        return $this->onRelationInlineManageAdd();
    }

    public function onRelationInlineClickManageListPivot()
    {
        return $this->onRelationInlineManagePivotForm();
    }

    public function onRelationInlineClickViewList()
    {
    ...
    public function onRelationInlineManageForm()
    {
        $this->beforeAjax();
     ...
     */
    public function onRelationInlineManageCreate()
    {
        $this->forceManageMode = 'form';
        $this->beforeAjax();
        $saveData = $this->manageWidget->getSaveData();
        $sessionKey = $this->deferredBinding ? $this->relationInlineGetSessionKey(true) : null;
     ...
     */
    public function onRelationInlineManageDelete()
    {
        $this->beforeAjax();
        ...
        elseif ($this->viewMode === 'single') {
            ...
            $this->initInlineRelation($this->model);
     ...
     */
    public function onRelationInlineManageAdd()
    {
        $this->beforeAjax();
     ...
     */
    public function onRelationInlineManageRemove()
    {
        $this->beforeAjax();
     ...
     */
    public function onRelationInlineManageAddPivot()
    {
        return $this->onRelationInlineManagePivotForm();
    }

    public function onRelationInlineManagePivotForm()
    {
        $this->beforeAjax();

        $this->vars['foreignId'] = $this->foreignId ?: post('checked');

        return $this->relationInlineMakePartial('pivot_form');
    }

    public function onRelationInlineManagePivotCreate()
    {
        $this->beforeAjax();
        ...
        return ['#'.$this->relationInlineGetId('view') => $this->relationInlineRenderView()];
    }

    public function onRelationInlineManagePivotUpdate()
    {
        $this->beforeAjax();
     ...
     */
    public function relationInlineExtendConfig($config, $field, $model)
    {
    }
     ...
     * @param \Winter\Storm\Database\Model $model
     */
    public function relationInlineExtendViewWidget($widget, $field, $model)
    {
    }
     ...
     * @param \Winter\Storm\Database\Model $model
     */
    public function relationInlineExtendManageWidget($widget, $field, $model)
    {
    }
     ...
     * @param \Winter\Storm\Database\Model $model
     */
    public function relationInlineExtendPivotWidget($widget, $field, $model)
    {
    }
     ...
     * @param \Winter\Storm\Database\Model $model
     */
    public function relationInlineExtendManageFilterWidget($widget, $field, $model)
    {
    }
     ...
     * @param \Winter\Storm\Database\Model $model
     */
    public function relationInlineExtendViewFilterWidget($widget, $field, $model)
    {
    }
     ...
     * @return array
     */
    public function relationInlineExtendRefreshResults($field)
    {
    }
    ...
    protected function makeConfigForMode($mode = 'view', $type = 'list', $throwException = true)
    {
        ...
        if (!$config) {
            ...
            if ($throwException) {
                throw new ApplicationException('Missing configuration for '.$mode.'.'.$type.' in RelationInlineController definition '.$this->field);
            }
     ...
     */
    public function relationInlineGetManageWidget()
    {
     ...
     * @return \Backend\Classes\WidgetBase
     */
    public function relationInlineGetViewWidget()
    {
