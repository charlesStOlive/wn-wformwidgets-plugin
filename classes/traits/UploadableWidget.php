...
trait UploadableWidget
{
    ...
    public function uploadableGetUploadPath(string $fileName): string
    {
        ...
        if($this->uploadPath == 'auto') {
            $class_name = get_class($this->model);
            $modelName = strtolower(substr($class_name, strrpos($class_name, '\\') + 1));
            $modelId = $this->model->id ?? 'new';
            $uuid = TinyUuid::generateFromDate();
            $this->uploadPath = sprintf('/uploaded-files/%s/%s/%s', $modelName,$modelId, $uuid);
        }
